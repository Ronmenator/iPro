...
interface ManuscriptStore {
  ...
  // Clear methods
  clearAllChapters: () => Promise<void>;
  clearAllScenes: () => Promise<void>;
...
export const useManuscriptStore = create<ManuscriptStore>((set, get) => ({
  ...
  deleteChapter: async (chapterId) => {
    const { db, chapters, chapterOrder } = get();
    chapters.delete(chapterId);
    const newChapterOrder = chapterOrder.filter(id => id !== chapterId);
    set({ chapters: new Map(chapters), chapterOrder: newChapterOrder });
  ...
  getAllChapters: () => {
    const { chapterOrder, chapters } = get();
    return chapterOrder.map(id => chapters.get(id)).filter(Boolean) as ChapterMetadata[];
  },
  ...
  reorderChapters: async (chapterIds) => {
    set({ chapterOrder: chapterIds });
    await get().saveStructure();
  },

  // Structure
  getManuscriptStructure: () => {
    const { chapters, scenes, chapterOrder } = get();
    return {
      chapters,
      scenes,
      chapterOrder,
  ...
  loadStructure: async () => {
    ...
    set({
      ...
      scenes: scenesMap,
      chapterOrder: structure?.chapterOrder || [],
    });
  },
  ...
  clearAllChapters: async () => {
    const { db, chapters, chapterOrder } = get();
    
    // Clear from memory
    chapters.clear();
    set({ chapters: new Map(), chapterOrder: [] });
